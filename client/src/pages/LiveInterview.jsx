...
import { getInterview, startInterview, submitAnswer } from "../services/api";
...
function LiveInterview() {
  ...
  useEffect(() => {
    const loadInterview = async () => {
      try {
        ...
        const backendInterview = data.interview;

        setInterview(backendInterview);

        // --------------------------------
        // Already completed?
        // --------------------------------

        if (backendInterview.status === "completed") {
        ...
        if (backendInterview.status === "in-progress") {
          const index = backendInterview.currentQuestionIndex;

          const question = backendInterview.questions[index];
        ...
        const startedInterview = started.interview;

        setInterview((previous) => ({
          ...previous,
          status: startedInterview.status,

          currentQuestionIndex: startedInterview.currentQuestionIndex,
        }));

        setCurrentQuestionIndex(startedInterview.currentQuestionIndex);

        setCurrentQuestion(startedInterview.currentQuestion);
      } catch (error) {
  ...
  const totalQuestions = interview?.totalQuestions || 0;
  ...
  const handleSubmit = async () => {
    if (!answer.trim() || submitting || !currentQuestion) {
      return;
    }
    ...
    try {
      ...
      const data = await submitAnswer(interviewId, answer.trim());
      ...
      <main className="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-10">
        ...
        <div className="mt-8 text-center">
          <p className="text-xs text-slate-400">
            Take your time. The AI interviewer will evaluate your answer after
            submission.
          </p>
        </div>
      </main>
